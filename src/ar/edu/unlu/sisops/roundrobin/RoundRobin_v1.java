...
public class RoundRobin_v1 {
    ...
    public static boolean roundRobin() {
        ...
        double sumaTiempoRestante = procesos.stream().mapToDouble(p -> p.tiempoRestante).sum();

        // Mientras queden procesos con tiempo restante por atender
        while ( sumaTiempoRestante > 0 ) {

            for ( Proceso proceso : procesos ) {

                // Si ya se terminó de atender el proceso, se sigue con el siguiente
                if ( proceso.tiempoRestante == 0 ) continue;
            ...
            sumaTiempoRestante = procesos.stream().mapToDouble(p -> p.tiempoRestante).sum();
        }
